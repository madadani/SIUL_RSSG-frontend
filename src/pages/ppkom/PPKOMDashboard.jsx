...
import { Search, Filter, RefreshCw, Eye, Ban, Check, ClipboardList, Clock } from 'lucide-react';
...
export default function PPKOMDashboard() {
  ...
  const processCount = usulanList.filter(u => u.status_kode === 'DIDISPOSISI_PP').length;
      ...
      <div className="flex gap-4 mb-6 flex-col md:flex-row w-full">
          <div onClick={() => navigate('/dashboard/riwayat_usulan?status=DIDISPOSISI_PPKOM')} className={`cursor-pointer rounded-2xl p-5 flex flex-col justify-between shadow-lg transition-all hover:-translate-y-1 border md:min-w-[240px] flex-1 ${isDarkMode ? 'bg-[#1e293b] border-slate-700/50' : 'bg-white border-gray-200'}`}>
            <div className="flex justify-between items-start">
              <div>
                <p className={`text-xs font-bold tracking-wide mb-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>MENUNGGU DISPOSISI PP</p>
          ...
          <div onClick={() => navigate('/dashboard/riwayat_usulan?status=DIKEMBALIKAN_KE_PPKOM')} className={`cursor-pointer rounded-2xl p-5 flex flex-col justify-between shadow-lg transition-all hover:-translate-y-1 border md:min-w-[240px] flex-1 ${isDarkMode ? 'bg-[#1e293b] border-slate-700/50' : 'bg-white border-gray-200'}`}>
            <div className="flex justify-between items-start">
              <div>
                <p className={`text-xs font-bold tracking-wide mb-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>DIKEMBALIKAN KE PPKOM</p>
                        ...
                        {selectedId === item.id ? (
                          <div className={`flex flex-col space-y-2 p-3 rounded-2xl border backdrop-blur-md transition-all animate-in zoom-in-95 duration-200 min-w-[240px] ${isDarkMode ? 'bg-slate-800/80 border-indigo-500/30 shadow-xl' : 'bg-white border-indigo-100 shadow-md'}`}>
                            ...
                            >
                              ...
                              {ppUsers.map(u => <option key={u.id} value={u.id}>{u.nama}</option>)}
                            </select>
                            <input 
                              type="text" 
                              placeholder="Catatan..." 
